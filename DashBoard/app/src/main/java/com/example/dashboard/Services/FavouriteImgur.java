...
public class FavouriteImgur extends AppCompatActivity {
        ...
        httpClient.newCall(request).enqueue(new Callback()
        {
            ...
            public void onResponse(Call call, Response response) throws IOException
            {
                try
                {
                    ...
                    final List<FavouriteImgur.Photo> photos = new ArrayList<>();

                    for (int i = 0; i < items.length(); i++)
                    {
                        JSONObject item = items.getJSONObject(i);
                        FavouriteImgur.Photo photo = new FavouriteImgur.Photo();
    ...
    private void render(final List<FavouriteImgur.Photo> photos)
    {
        RecyclerView recyclerView = findViewById(R.id.rv_of_photos);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        RecyclerView.Adapter<FavouriteImgur.PhotoVH> adapter = new RecyclerView.Adapter<FavouriteImgur.PhotoVH>()
        {
            @NonNull
            @Override
            public FavouriteImgur.PhotoVH onCreateViewHolder(@NonNull ViewGroup parent, int viewType)
            {
                FavouriteImgur.PhotoVH vh = new FavouriteImgur.PhotoVH(getLayoutInflater().inflate(R.layout.list_image_imgur, null));
            ...
            @Override
            public void onBindViewHolder(@NonNull FavouriteImgur.PhotoVH holder, int position)
            {
                Picasso.get()
